import { marketplaces, type MarketplaceKey } from './marketplaces'

export const NETWORK_ANCHOR_MS = Date.UTC(2026, 7, 15, 12)
...
export interface SeedProgramOffer {
  ...
  /** Brand product this offer promotes. Offers for one product on several marketplaces share it. */
  productKey: string
  /** The listing's identifier on its marketplace: child ASIN, Walmart item id, Shopify variant id. */
  externalId: string
  priceCents: number
...
export const advertisers: SeedAdvertiser[] = advertiserSeeds.map((advertiser) => ({
  ...advertiser,
  logoUrl: `/network/catalog/brands/${advertiser.key}.png`,
}))

type OfferSeed = Omit<SeedProgramOffer, 'productImageUrl' | 'productKey'> & { productKey?: string }
...
const offerSeeds: OfferSeed[] = [
  ...
  },
  {
    ...
    attributionWindowDays: 30,
    featured: false,
    externalId: '934604251',
    priceCents: 3_699,
    commissionRateBps: 3000,
    marketplaceKey: 'walmart-us',
    rating: 4.8,
    reviewCount: 1_902,
    access: 'partnered',
    isDeal: false,
...
export const programOffers: SeedProgramOffer[] = offerSeeds.map((offer) => ({
  ...offer,
  productKey: offer.productKey ?? offer.key,
  productImageUrl: `/network/catalog/products/${offer.key}.jpg`,
}))
...
export const conversions: SeedConversion[] = Array.from({ length: 180 }, (_, index) => {
  ...
  return {
    ...
    offerKey: offer.key,
    linkKey: link.key,
    occurredAt: NETWORK_ANCHOR_MS - (index % 90) * DAY_MS - (index % 19) * 1_800_000,
    status,
    orderValueCents,
    grossCommissionCents,
    publisherEarningsCents,
...
export const networkExceptions = [
...
export const seedCounts = {
  ...
  products: new Set(programOffers.map((offer) => offer.productKey)).size,
  listings: programOffers.length,
