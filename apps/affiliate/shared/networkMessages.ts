export type NetworkMessageIdentity = 'operator' | 'northstar' | 'everyday' | 'avery' | 'puroair'

export interface NetworkMessageParticipantSeed {
  identityKey: NetworkMessageIdentity
  title: string
  team: string
  unreadCount: number
}

export interface NetworkMessageSeed {
...
export interface NetworkMessageThreadSeed {
  key: string
  subject: string
  team: string
  status: 'online' | 'away'
  participants: NetworkMessageParticipantSeed[]
  messages: NetworkMessageSeed[]
}

const at = (day: number, hour: number, minute: number) => Date.UTC(2026, 7, day, hour, minute)

export const networkMessageThreads: NetworkMessageThreadSeed[] = [
  {
