...
describe('deterministic network data', () => {
  ...
  test('every catalog record has deterministic local brand and product photography', () => {
    for (const advertiser of advertisers) {
      expect(advertiser.logoUrl).toBe(`/network/catalog/brands/${advertiser.key}.png`)
  ...
  test('message fixtures connect operator, publisher, and creator workspaces', () => {
    expect(new Set(networkMessageThreads.map((thread) => thread.key)).size).toBe(
      networkMessageThreads.length,
    )
    expect(networkMessageThreads.length).toBeGreaterThanOrEqual(9)
    for (const identityKey of ['operator', 'northstar', 'everyday', 'avery', 'puroair'] as const) {
      expect(
        networkMessageThreads.some((thread) =>
