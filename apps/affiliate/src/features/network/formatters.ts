import { conversions } from '../../../shared/networkData'
...
export function scopeFactor(identity: NetworkIdentity) {
  if (identity === 'northstar') return 0.186
  if (identity === 'everyday') return 0.024
  return 1
}

export function publisherKeyForIdentity(identity: NetworkIdentity) {
  if (identity === 'northstar') return 'northstar-media'
  if (identity === 'everyday') return 'everyday-finds'
  return null
}

export function scopedConversions(identity: NetworkIdentity) {
